...
function renderCategory(category){
 ...
 const cards=items.map(a=>{ const [cat,slug,title,focus]=a; const url=articleUrl(cat,slug); return `<article class="blog-card"><a class="blog-card-image-link" href="${url}" aria-label="Read ${esc(title)}"><img class="blog-card-image" src="${imageFor(slug)}" width="1200" height="650" loading="lazy" decoding="async" alt="${esc(title)}"></a><div class="blog-card-content"><h2><a href="${url}">${esc(title)}</a></h2><p>${esc(focus.charAt(0).toUpperCase()+focus.slice(1))}</p><a href="${url}">Read article</a></div></article>`; }).join('');
